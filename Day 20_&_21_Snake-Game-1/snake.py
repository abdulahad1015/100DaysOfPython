...
class Snake():
    def __init__(self):
        ...
        self.draw.penup()
        self.draw.goto(-50, 275)
        self.draw.write(f"Score = {self.score}", font={"calibri", 16, "normal"})
        # Generate snake
        for i in range(3):
    ...
    def snake_point(self):
        ...
        if (self.segments[0].pos() == self.point.pos()):
            ...
            self.score += 10
            self.draw.clear()
            self.draw.write(f"Score = {self.score}", font={"calibri", 18, "normal"})

    def gameover(self):
        self.draw.home()
        self.draw.backward(50)
        self.draw.color("yellow")
        self.draw.write("Game Over", font={"calibri", 28, "normal"})
